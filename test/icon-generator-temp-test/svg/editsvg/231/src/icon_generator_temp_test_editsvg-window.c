...
change_function(){
  ...
  GFileIOStream *gfiostream;
  GFile *temp = g_file_new_tmp (NULL, gfiostream, NULL);
  //move to temp
  gssize wrote;
  wrote = g_output_stream_write (gfiostream, buffer, 10, NULL, NULL);
  /*
   some work and changes
  */
  //move from temp
  GFileInputStream *gfistream = g_file_read (temp, NULL, NULL);
